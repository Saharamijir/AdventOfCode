...
let main _ =
  let data = LoadData
  let firstWire = GetPoints data.[0]
  let secondWire = GetPoints data.[1]
  let intersections =
    Set.intersect
        (firstWire |> Set.ofSeq)
        (secondWire |> Set.ofSeq)
    |> Set.filter (fun x -> x <> (0,0))
  let mutable sequence = Seq.empty
  for x in intersections do
    sequence <- Seq.append sequence [(firstWire.IndexOf x) + (secondWire.IndexOf x)]
  sequence |> Seq.min |> printfn "%i"
